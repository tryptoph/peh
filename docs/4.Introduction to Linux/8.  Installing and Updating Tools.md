Here are explanations and e\xamples of the commands mentioned in this video:
...
>[!info]
>- linux also need update (patching)
>`sudo apt update && apt upgrade` (install : go to repositories::look through packages ; update ; update available)
>- `sudo su -` must be root to run some commands rather than sudo
>- updating can break things (so have no/have back up of your vm)
>- `sudo git clone `(pimpmykali)(after using :no to move kali folder what inside it ..to root folder)