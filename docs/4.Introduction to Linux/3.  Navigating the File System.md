...
>[!info]
>- kali(user)@kali(hostname=computername)
>[~] directory i am in
>- colors brought by ls (file and directory)
>- tab
>- u can cd from any folder with the **full path** or go only to paths u see with ls directly
>- see files of any folder without being in it 
>```bash
>ls /etc
>```
