...
>[! info]
>- d for directory (blue),file (white),link(light blue)
>- there 3 groups : owner of file, group membership, other users
>- create text file hello.txt ... see permissions(text file not have x :is it by default or not executable(by default may be for all new ))
>- use `chmod`  2 ways: 
>```shell
>chmod +rwx hello.txt
>chmod 777 hello.txt
>```
>![[2023-08-16 00_46_25-Window.png]]
>- `adduser , su , cat etc/passwd`(used inpast to store passwords , now are stored in shadow file in form of hashes (`sudo cat etc/shadow`))
>- root not show its hash , but kali and john are shown .. even same password , they have different hash (only in linux )...
>- sudo `cat etc/sudoers`
>- grep 'sudo' `etc/group`
>- `sudo -l` (see comands i can run)
