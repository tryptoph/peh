...
>[!Note]
>TCP and UDP are a layer 4 protocol

>[!Example]
>If u want to connect to port 443 in website , then u send SYN(Hi i want to connect to port 443) ,if available SYN-ACK (connect to me) packet is returned then u send ACK(establish the connection) packet to show that u're ready to go .

- modify 3 way handshake for scanning
